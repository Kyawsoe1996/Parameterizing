...
for (def row = 1; row <= findTestData('New Test Data for Testcase').getRowNumbers(); row++) {
    Login(findTestData('New Test Data for Testcase').getValue('UserName', row), findTestData('New Test Data for Testcase').getValue(
            'Password', row))
...
def Login(def userName, def password) {
	...
	WebUI.setText(myTestObject,"customObject");
	
//	 WebUI.setText(findTestObject('Page_Execute Automation/input_UserName', [("xpath") : "id('userName')/p[1]/input[1]"]), 
//        userName)
